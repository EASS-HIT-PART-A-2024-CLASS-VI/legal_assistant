...
PLATFORM_MODELS = {
    ...
    PlatformType.OPENAI: {
        "default_embedding": {
            ...
            "model_mode": OpenAIEmbeddingMode.SIMILARITY_MODE,
            "dimensions": 512,
        },
        "default_client": {
...
def get_client(platform: PlatformType = PlatformType.OPENAI):
    ...
    match platform:
        case PlatformType.Gemini:
            return __get_gemini_client(
                model_name=client_conf["model_name"],
                temperature=client_conf["temperature"],
                max_tokens=client_conf["max_tokens"],
            )
        case PlatformType.OPENAI:
            return __get_openai_client(
                model_name=client_conf["model_name"],
                temperature=client_conf["temperature"],
                max_tokens=client_conf["max_tokens"],
            )
...
def get_embedding_client(platform: PlatformType = PlatformType.OPENAI):
    ...
    match platform:
        ...
        case PlatformType.OPENAI:
            return __get_openai_embedding(
                model_name=embedding_conf["model_name"],
                model_mode=embedding_conf["model_mode"],
                dimensions=embedding_conf["dimensions"],
            )
        case _:
